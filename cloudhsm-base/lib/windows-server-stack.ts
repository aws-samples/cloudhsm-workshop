...
export class WindowsServerStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: WindowsServerStackProps) {
    ...
    const assetBucketName =
      props.assetsBucketName || bootstrapScriptAsset.s3BucketName;
    const assetObjectKey = bootstrapScriptAsset.s3ObjectKey;
    const assetBucketPrefix =
      props.assetsBucketPrefix || assetObjectKey.split('/')[0]; // Get the prefix part
    ...
    const deploymentVersion =
      this.node.tryGetContext('deploymentVersion') || new Date().toISOString();
    ...
    if (props.assetsBucketName && props.assetsBucketPrefix) {
      ...
      console.log(
        `Using workshop assets for bootstrap script: ${bootstrapScriptUrl}`,
      );
    }
      ...
      scriptS3Urls: Object.fromEntries(
        additionalScripts.map((script, idx) => {
          ...
          if (props.assetsBucketName && props.assetsBucketPrefix) {
            ...
            return [
              script,
              `s3://${props.assetsBucketName}/${formattedPrefix}scripts/windows/${script}`,
            ];
          }
...
`;
    ...
    const userData = UserData.custom(
      `<powershell>${scriptContent}</powershell>`,
    );

    // Ensure the security group allows all outbound traffic
    props.securityGroup.addEgressRule(
      Peer.anyIpv4(),
      Port.allTraffic(),
    ...
    new cdk.CfnOutput(this, 'PublicDns', {
      value:
        ...
        'N/A (Instance might be in private subnet)',
    });
